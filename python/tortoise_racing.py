...
average speed of ```720 feet per hour```.
Young ***B*** knows she runs faster than ***A***, and furthermore has not
...
More generally:
given two speeds `v1` (***A***'s speed, integer > 0) and `v2` (***B***'s speed,
integer > 0) and a lead `g` (integer > 0)
how long will it take ***B*** to catch ***A***?
...
C++, C, Go, Nim, Pascal, COBOL, Erlang, `[-1, -1, -1]` for Perl,`[]` for Kotlin
or "-1 -1 -1" for others.
...
def race(v1, v2, g):
    ...
    return list(
        map(
            int,
            str(dt.timedelta(seconds=(g / (v2 - v1)) * 3600)).split(".")[0].split(":"),
        )
    )
