...
You have to do something, so that all the changes done on the documents 
table are reflected in the documents_changelog table:
...
Note: the id column of the documents table is autoincrementing, hence when new records 
will be inserted there by the test code, the id's will not be provided 
explicitly - some language features which could be used for solving 
